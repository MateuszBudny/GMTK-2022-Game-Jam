...
Shader "LowkeyShadersURP/Lit/MonteCristoLitByScreen"
{
    Properties
    {
        ...
        [ToggleOff] _WarpEffect("Warp Effect", Float) = 0 // added by @mateuszbudny
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _WarpEffect; // added by @mateuszbudny
            ...
            Varyings LitPassVertex(Attributes input)
            {
                ...
                // modified by @mateuszbudny:
                float4 rawScreenPos = ComputeScreenPos(mul(UNITY_MATRIX_MVP, input.positionOS)); // screen pos warped by object shape - only surfaces parallel to the screen are not warped
                float2 screenPos = (rawScreenPos.xy / (_WarpEffect == 0 ? rawScreenPos.w : 1)); // dividing by rawScreenPos.w gets rid of the warped effect
                output.uv = float2(screenPos.x / _BaseMap_ST.x, screenPos.y / _BaseMap_ST.y) - _BaseMap_ST.zw;
